...
module.exports = async function doBlueskyPost() {
  ...
  // Only proceed if user explicitly provided --bluesky
  if (!process.argv.includes('--bluesky')) {
    log.info('[Bluesky Feed] No --bluesky parameter found. Skipping Bluesky posting.');
    return;
  }

  log.info('[Bluesky Feed] Generation finished. Preparing Bluesky update...');
