...
module.exports = async function(args) {
  ...
  // Invalidate locals to force a fresh rebuild.
  const locals = this.locals;
  if (typeof locals.invalidate === 'function') {
    locals.invalidate();
    log.info("Hexo locals invalidated");
  }

  // Debug: Log the entire locals object as a plain object.
  log.info("DEBUG: Hexo locals as object:", locals.toObject());
  ...
  log.info("DEBUG: Hexo locals:", locals);
  const postsCollection = locals.get('posts');
  if (!postsCollection) {
    log.error("locals.get('posts') returned undefined");
    return;
  }
  log.info("DEBUG: Posts collection (raw):", postsCollection);
  ...
  log.info("DEBUG: Number of posts found: " + postsArray.length);

  if (postsArray.length === 0) {
    log.error("No posts found in Hexo locals.");
    return;
  }
  // Sort posts by their date field descending using the Moment instance's toDate() if available
  postsArray.sort((a, b) => {
  ...
  log.info("DEBUG: Latest post object after sorting by date:", latestPost);
  ...
  if (urlStart !== -1) {
    ...
    log.info("DEBUG: Constructed facets:", facets);
  } else {
  ...
  try {
    ...
    log.info("DEBUG: Session response:", sessionResponse.data);
  } catch (error) {
